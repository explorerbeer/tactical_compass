...
[ModDB page](https://www.moddb.com/members/explorerbee) Ј
[Demonstration video](https://www.youtube.com/watch?v=boGvng0kotI) Ј
[–усска€ верси€](README.ru.md) Ј
[Changelog](CHANGELOG.md)

## Features

- **Two compass styles** Ч S.T.A.L.K.E.R. 2 and the classic one, switchable in MCM at any time.
- **Two marker icon sets** Ч custom icons by Rier, or vanilla PDA icons that follow your own icon addons.
...
- **Per-category settings** Ч enable or disable each type, set its display range, or make it
  always visible regardless of distance.
- **Distance to target**, shown when you look straight at a marker.
- **Elevation arrows** Ч a marker tells you whether the target is above or below you.
- **Distance fade** Ч markers fade out smoothly instead of popping in and out.
...
Optional Ч each one unlocks the corresponding marker type:
...
renamed in v5, and leftovers from the old version will keep running alongside the new ones Ч
...
- **Main settings** Ч compass style, position on screen (top or bottom), scale.
- **Markers** Ч icon set, drop shadow, and four options for each of the nine categories:
  display, display distance to target, always display, and display range.
- **Key binding** Ч the toggle key.
...
- The minimap reappears after changing screen resolution. This is an engine bug Ч it
...
- **VodoXleb**, **xcvb (bvcx)**, **RavenAscendant** Ч scripting help
- **morrazzzz** Ч found the `actor_map_location_added` callback the marker system is built on
- **EZ4E99** Ч compass texture
- **Rier** (RierSenpai) Ч custom marker icons ([DeviantArt](https://www.deviantart.com/riersenpai))